...
Module Name: ADx
online version: https://github.com/gromedev/adx/blob/main/module/help/Get-ADxGroup.md
schema: 2.0.0
---
...
Get Active Directory groups. Drop-in replacement for RSAT's `Get-ADGroup` over raw LDAP.

## SYNTAX

### Filter (Default)
```
Get-ADxGroup -Filter <String> [-Properties <String[]>] [-SearchBase <String>] [-SearchScope <String>]
...
### Identity
```
Get-ADxGroup [-Identity] <Object> [-Properties <String[]>] [-SearchBase <String>] [-SearchScope <String>]
 [-ResultSetSize <Int32>] [-ResultPageSize <Int32>] [-AllowUnknownProperty] [-Server <String>] [-Port <Int32>]
 [-UseSsl] [-Credential <PSCredential>] [-AuthType <String>] [-SearchTimeout <Int32>] [-ChaseReferrals]
 [-ProgressAction <ActionPreference>] [<CommonParameters>]
```
...
### LdapFilter
```
Get-ADxGroup -LDAPFilter <String> [-Properties <String[]>] [-SearchBase <String>] [-SearchScope <String>]
 [-ResultSetSize <Int32>] [-ResultPageSize <Int32>] [-AllowUnknownProperty] [-Server <String>] [-Port <Int32>]
 [-UseSsl] [-Credential <PSCredential>] [-AuthType <String>] [-SearchTimeout <Int32>] [-ChaseReferrals]
 [-ProgressAction <ActionPreference>] [<CommonParameters>]
```
...
Reading full membership of large groups is a job for `Get-ADxGroupMember` (range retrieval:
...
### Example 1
```powershell
PS C:\> Get-ADxGroup 'Domain Admins'
```

Gets one group by sAMAccountName. DN, objectGUID and SID identities also work.

### Example 2
```powershell
PS C:\> Get-ADxGroup -Filter "GroupCategory -eq 'Security' -and GroupScope -eq 'Universal'"
```

All universal security groups, filtered server-side via groupType bitwise matching rules.

### Example 3
```powershell
PS C:\> Get-ADxGroup -Filter "Name -like 'SG-*'" -Properties ManagedBy, Description
```

Prefix-named groups with their owner and description on top of the default set.
...
### -Filter
Query in RSAT's expression syntax; `-Filter *` matches every group. The group synthetics
`GroupScope` and `GroupCategory` are filterable with `-eq`/`-ne`. See `Get-ADxUser` for the
supported operator set and rejection rules.
...
### -LDAPFilter
Raw LDAP filter, ANDed with `(objectCategory=group)`.
...
### -ResultSetSize
Maximum number of groups to return; 0 (default) is unlimited, matching RSAT.
...
One object per group, typed `ADx.Group`. Default properties: DistinguishedName, GroupCategory,
GroupScope, Name, ObjectClass, ObjectGUID, SamAccountName, SID.

## NOTES

`MemberOf` excludes the primary group, exactly as RSAT's does - primary membership lives in
`primaryGroupID` on the member, not in the group's `member` attribute.
