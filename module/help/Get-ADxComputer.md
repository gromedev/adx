...
Module Name: ADx
online version: https://github.com/gromedev/adx/blob/main/module/help/Get-ADxComputer.md
schema: 2.0.0
---
...
Get Active Directory computers. Drop-in replacement for RSAT's `Get-ADComputer` over raw LDAP.

## SYNTAX

### Filter (Default)
```
Get-ADxComputer -Filter <String> [-Properties <String[]>] [-SearchBase <String>] [-SearchScope <String>]
...
### Identity
```
Get-ADxComputer [-Identity] <Object> [-Properties <String[]>] [-SearchBase <String>] [-SearchScope <String>]
 [-ResultSetSize <Int32>] [-ResultPageSize <Int32>] [-AllowUnknownProperty] [-Server <String>] [-Port <Int32>]
 [-UseSsl] [-Credential <PSCredential>] [-AuthType <String>] [-SearchTimeout <Int32>] [-ChaseReferrals]
 [-ProgressAction <ActionPreference>] [<CommonParameters>]
```
...
### LdapFilter
```
Get-ADxComputer -LDAPFilter <String> [-Properties <String[]>] [-SearchBase <String>] [-SearchScope <String>]
 [-ResultSetSize <Int32>] [-ResultPageSize <Int32>] [-AllowUnknownProperty] [-Server <String>] [-Port <Int32>]
 [-UseSsl] [-Credential <PSCredential>] [-AuthType <String>] [-SearchTimeout <Int32>] [-ChaseReferrals]
 [-ProgressAction <ActionPreference>] [<CommonParameters>]
```
...
Reading full membership of large groups is a job for `Get-ADxComputerMember` (range retrieval:
...
### Example 1
```powershell
PS C:\> Get-ADxComputer WS01
```

Gets one computer by name. The stored sAMAccountName is `WS01$`; the suffix is tried
automatically. DN, objectGUID and SID identities also work.

### Example 2
```powershell
PS C:\> Get-ADxComputer -Filter "OperatingSystem -like '*Server*'" -Properties OperatingSystem, OperatingSystemVersion
```

All servers by OS string, with version details on top of the default set.

### Example 3
```powershell
PS C:\> $stale = (Get-Date).AddDays(-60)
PS C:\> Get-ADxComputer -Filter "Enabled -eq $true -and LastLogonDate -lt $stale" -Properties LastLogonDate
```

Enabled computer accounts that have not authenticated in 60 days - the classic stale-machine
sweep, filtered server-side.
...
### -Filter
Query in RSAT's expression syntax; `-Filter *` matches every computer. See `Get-ADxUser` for
the supported operator set and rejection rules.
...
### -Identity
A distinguished name, objectGUID, SID, or sAMAccountName, detected in that order. See
`Get-ADxUser`.
...
### -LDAPFilter
Raw LDAP filter, ANDed with `(objectCategory=computer)`.
...
### -ResultSetSize
Maximum number of computers to return; 0 (default) is unlimited, matching RSAT.
...
One object per computer, typed `ADx.Computer`. Default properties: DistinguishedName,
DNSHostName, Enabled, Name, ObjectClass, ObjectGUID, SamAccountName, SID, UserPrincipalName.

## NOTES

`IPv4Address`/`IPv6Address` are not supported in `-Properties`: RSAT computes them
client-side via DNS, not from the directory. Asking for them is an explicit error rather
than a silent null column; resolve `DNSHostName` yourself if you need addresses.
